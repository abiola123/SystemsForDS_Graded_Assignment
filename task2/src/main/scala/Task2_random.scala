...
//     /* TODO: this is only valid if you store on exacly 5 or 6 nodes, and save the data to some node, else REMOVE DOWN */
//     for (i <- (0 until 1036)) { // about 1036 or 36 memory left free
//       m = router.sendMessage(USER, s"u${0}", new Message(USER, STORE, s"keyA${i}->valueA${i}")) // STORE_FAILURE or STORE_SUCCESS, but occupies space
//       println(s"${0}: " + m.messageType + " " + m.data)
//     }
//     for (i <- (0 until 1000)) {
//       m = router.sendMessage(USER, s"u${i}", new Message(USER, STORE, s"key${1000 + i}->value${1000 + i}")) // STORE_FAILURE
//       println(s"${i}: " + m.messageType + " " + m.data)
//       assert(m.messageType == STORE_FAILURE)
//     }
//     /* TODO: this is only valid if you store on exacly 5 or 6 nodes, and save the data to some node, else REMOVE UP */
...
//     }
//     for (i <- (0 until 1000)) {
//       m = router.sendMessage(USER, s"u${i}", new Message(USER, RETRIEVE, s"key${1000 + i}")) // RETRIEVE_FAILURE
//       println(s"${i}: " + m.messageType + " " + m.data)
//       assert(m.messageType == RETRIEVE_FAILURE)
//     }
